...
- A 150-item decomposition stress diagnostic selected the lowest exact-provenance evidence items from three held-out Cohere cells and compared five parsing policies. Assertion-guarded query-aware multi-span retrieval recovered 83/150 auditable supports, compared with 24/150 for normalized full-note matching and 0/150 for exact full-note matching, while using far less context on supported items. A coherence audit splits those 83 supports into 54 auto-accepted items and 29 review-required items, with 0 high-risk supported items and 67 abstentions. This is failure-analysis evidence, not a population estimate or proof that chunking solves semantic fidelity.

...
- A 150-item decomposition stress diagnostic selected the lowest exact-provenance evidence items from three held-out Cohere cells and compared five parsing policies. Query-aware multi-span retrieval recovered 101/150 auditable supports, compared with 24/150 for normalized full-note matching and 0/150 for exact full-note matching, while using far less context on supported items. It abstained on all 26 low-overlap possible-fabrication items in the stress set. This is failure-analysis evidence, not a population estimate or proof that chunking solves semantic fidelity.

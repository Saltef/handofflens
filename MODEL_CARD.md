...
- A 60-item decomposition stress diagnostic selected the lowest exact-provenance evidence items from three held-out Cohere cells and compared five parsing policies. Targeted line/query-aware retrieval recovered 15/60 auditable supports, compared with 8/60 for normalized full-note matching and 0/60 for exact full-note matching, while using far less context on supported items. This is failure-analysis evidence, not a population estimate or proof that chunking solves semantic fidelity.

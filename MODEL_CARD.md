...
- Hosted structured-output routes did not enforce every local JSON-schema constraint, including the max-3 span cap; cap violations require local validation plus a retry or repair policy before being counted as clean automated outputs.
...
- Add a cap-violation retry or repair loop for span-ID outputs, then compare an embedding/reranker-backed span matcher against the current lexical-matcher path under matched token and item-count budgets.

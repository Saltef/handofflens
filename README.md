...
A targeted decomposition stress test then selected the 50 lowest exact-provenance evidence items from each of three held-out Cohere cells and compared five parsing policies. Across 150 deliberately difficult items, exact full-note matching supported 0/150, normalized full-note matching supported 24/150, simple line-span retrieval supported 32/150, section-filtered retrieval supported 26/150, and assertion-guarded query-aware multi-span retrieval supported 83/150. A coherence audit then split those 83 supported items into 54 auto-accepted supports and 29 review-required supports; 67/150 items abstained, and 0 high-risk supported items remained. The key tradeoff is context cost: normalized full-note support consumed about 1,504 source words when it succeeded, while line-span and query-aware retrieval used about 10.1 and 20.0 words respectively. This supports a parsing/chunking experiment for dense notes, but it does not prove that length causes failure or that retrieval fixes semantic errors.

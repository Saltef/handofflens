...
A targeted decomposition stress test then selected the 100 lowest exact-provenance evidence items from each of three held-out Cohere cells and compared five parsing policies. Across 300 deliberately difficult items, exact full-note matching supported 0/300, normalized full-note matching supported 35/300, simple line-span retrieval supported 55/300, section-filtered retrieval supported 41/300, and assertion-guarded clause-aware query retrieval supported 188/300. A coherence audit then split those 188 supported items into 106 auto-accepted supports and 82 review-required supports; 112/300 items abstained, and 0 high-risk supported items remained. The latest revision adds target-aware assertion-cue scope checks, recovering 21 items previously blocked by broad cue matching. The tradeoff is that most of those new recoveries require review rather than automatic acceptance. Normalized full-note support consumed about 1,676 source words when it succeeded, while line-span and query-aware retrieval used about 9.2 and 22.3 words respectively. This supports a parsing/chunking experiment for dense notes, but it does not prove that length causes failure or that retrieval fixes semantic errors.

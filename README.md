...
A targeted decomposition stress test then selected the 50 lowest exact-provenance evidence items from each of three held-out Cohere cells and compared five parsing policies. Across 150 deliberately difficult items, exact full-note matching supported 0/150, normalized full-note matching supported 24/150, simple line-span retrieval supported 32/150, section-filtered retrieval supported 26/150, and query-aware multi-span retrieval supported 101/150. The key tradeoff is context cost: normalized full-note support consumed about 1,504 source words when it succeeded, while line-span and query-aware retrieval used about 10.1 and 22.2 words respectively. The query-aware method explicitly abstained on all 26 low-overlap possible-fabrication items in this stress set. This supports a parsing/chunking experiment for dense notes, but it does not prove that length causes failure or that retrieval fixes semantic errors.

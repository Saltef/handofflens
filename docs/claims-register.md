...
| Live-retry recovery of contract-invalid items (`span-id-retry-recovery-v1`) | Across 3 repeats, a single corrective retry recovered 78.8% [72.5, 84.1] of Command A+ too-many-span items and 100% [98.9, 100] of Haiku's, preserving item and supported-item counts (paired Delta ~= 0); a repeats=1 sample had optimistically read 100% for both, so retry reduces but does not eliminate the cap-repair burden for Command A+ | Claiming one retry guarantees a compliant output (Command A+ leaves ~21% unrecovered), claiming retry proves semantic correctness or gold recall, or ignoring its per-call latency/token cost |
...
| End-to-end cardinality-constraint experiment (`structured-output-maxitems-causal-v1`) | Both providers reject `maxItems` on array types in structured output (deterministic 400), so the span cap cannot be enforced via schema; a prompt-level cap causally lowers the too-many-span rate on both providers under a paired single-variable design, but a recall-proxy rerun shows the paired item drop is concentrated in supported items (Delta supported ~= Delta total, Delta unsupported ~= 0), i.e. the reduction comes from lost coverage, not shed over-extraction | Claiming the prompt cap is a quality gain (it drops supported items rather than trimming over-extraction), claiming schema rejection is Cohere-specific, treating the provenance-support proxy as adjudicated gold recall, or reading violation-rate reduction as semantic grounding or model superiority |
...
| Support-score bake-off under CRC (`support-score-bakeoff-v1`) | On a synthetic adversarial pool, entailment > cue-aware > lexical for support discrimination under a fixed marginal false-acceptance budget; with a schema-aligned definition and cross-phrasing calibration examples, the Command A+ entailment judge (v2) reaches perfect separation (present -> 1, all distractor kinds incl. subtle -> 0), coverage 0.500 at zero realized risk, where lexical/cue-aware collapse to ~0; a v1 narrow-definition judge located the ceiling as task specification, not model capability | Claiming real clinical support detection, treating the LLM judge as ground truth, reading perfect separation as general skill (synthetic construction, judge evaluated on the distribution it was defined for), or reading absolute coverage as realistic (adversarial 50/50 base rate) |
| FDR-controlled selection on the support decision (`conformal-support-v1`, conformal selection) | On synthetic construction-true labels, conformal selection (Jin & Candes 2023: BH on conformal p-values) controls the false discovery rate among accepted items -- the decision-relevant purity of the auto-accept set -- at or below target q with case-clustered splits (realized FDR 0.000/0.004/0.012 at q 0.05/0.10/0.20; 0.4% FDR at full power for q=0.10); Monte-Carlo validated | Claiming real clinical FDR (labels are synthetic), ignoring conservativeness at strict q from a small calibration-null count (~18/split), or treating a synthetic-calibrated bound as valid for real notes without adjudicated labels |
| Conformal risk control on the support decision (`conformal-support-v1`) | On synthetic construction-true labels, CRC controls the marginal false-acceptance rate -- fraction of ALL items wrongly accepted, not the FDR among accepted -- at or below a chosen alpha with case-clustered splits (realized marginal risk 0.030/0.078/0.121 at alpha 0.05/0.10/0.20) and an explicit coverage cost; the offline Monte-Carlo guard validates the CRC implementation | Reading the marginal guarantee as a false-discovery-rate bound among accepted items, claiming real clinical coverage (labels are synthetic), reading the high supported-recall as general support-detection skill (the lexical score matched the only error mode present), or treating a guarantee calibrated on synthetic data as valid for real notes without recalibration on adjudicated labels |

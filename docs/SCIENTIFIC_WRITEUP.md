...
| Span-ID v5 cross-provider ablation | Are stable source-span IDs better than generated free-text quotes? | Cohere span-ID-v5 improved item support over quote-v2, while Haiku span-ID-v5 underperformed quote-v2; both directions persisted under matched-count comparison. | Raw case gates are volume-confounded. Span IDs improve pointer integrity and error accounting, but hosted schema constraints and semantic entailment remain unresolved. |
...
- Hosted-provider structured-output compatibility required stripping some local schema constraints, including the max-3 evidence-span cap. Cap violations therefore remain local validation failures unless a retry or repair loop is added.
...
3. Schema-enforcement and reranker follow-up. The span-ID-only ablation has now run. The next measurement should add a cap-violation retry or repair loop, then compare the lexical matcher with an embedding or reranker-backed matcher under matched item-count, span-budget, and review-routing policies.

...
Because the lowest-performing cases were also dense in extracted claims, HandoffLens added a decomposition stress diagnostic rather than claiming a broad causal length effect. The diagnostic selected 50 failed exact-provenance evidence items from each of three held-out Cohere cells and compared five support policies: exact full-note matching, normalized full-note matching, line-span retrieval, section-filtered retrieval, and query-aware multi-span retrieval with a bounded greedy token-union fallback. Across 150 deliberately difficult items, exact matching supported 0, normalized full-note matching supported 24, line-span retrieval supported 32, section-filtered retrieval supported 26, and query-aware multi-span retrieval supported 101. Full-note normalization consumed about 1,504 source words when it succeeded; targeted line and query-aware retrieval consumed about 10.1 and 22.2 words. The query-aware method abstained on all 26 low-overlap possible-fabrication items in this stress set. The design implication is not "long notes cause failure." It is narrower: many failures are recoverable by decomposing dense notes into auditable spans, but low-overlap items and semantic-fidelity questions still need abstention, entailment scoring, or review.
...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Query-aware multi-span retrieval supported 101/150 worst selected items, versus 24/150 for normalized full-note matching and 0/150 for exact full-note matching; all 26 low-overlap possible-fabrication items abstained. | Parsing helps the recoverable subset with far less context, but it does not solve semantic-fidelity failures. |

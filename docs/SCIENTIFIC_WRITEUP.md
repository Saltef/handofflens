...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Assertion-guarded clause-aware query retrieval supported 167/300 worst selected items; coherence audit split these into 101 auto-accepted and 66 review-required supports, with 133 abstentions and 0 high-risk supported items. | Parsing helps the recoverable subset with far less context, but it does not solve semantic-fidelity failures. |

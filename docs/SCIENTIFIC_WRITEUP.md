...
3. Span-ID-only and reranker ablations. Generated quote text should be compared against a span-ID-only schema, and the current lexical span matcher should be compared with an embedding or reranker-backed matcher under matched item-count, span-budget, and review-routing policies.

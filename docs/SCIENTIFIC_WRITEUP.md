...
Because the lowest-performing cases were also dense in extracted claims, HandoffLens added a decomposition stress diagnostic rather than claiming a broad causal length effect. The diagnostic selected 100 failed exact-provenance evidence items from each of three held-out Cohere cells and compared five support policies: exact full-note matching, normalized full-note matching, line-span retrieval, section-filtered retrieval, and query-aware multi-span retrieval with a bounded greedy token-union fallback plus assertion-cue guarding. Across 300 deliberately difficult items, exact matching supported 0, normalized full-note matching supported 35, line-span retrieval supported 55, section-filtered retrieval supported 41, and assertion-guarded query-aware multi-span retrieval supported 158. A coherence audit then separated those 158 supported items into 92 auto-accepted supports and 66 review-required supports, with 0 high-risk supported items and 142 abstentions. Full-note normalization consumed about 1,676 source words when it succeeded; targeted line and query-aware retrieval consumed about 9.2 and 21.7 words. The design implication is not "long notes cause failure." It is narrower: many failures are recoverable by decomposing dense notes into auditable spans, but low-overlap items, assertion conflicts, medium-risk stitching cases, and semantic-fidelity questions still need abstention, entailment scoring, or review.
...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Assertion-guarded query-aware multi-span retrieval supported 158/300 worst selected items; coherence audit split these into 92 auto-accepted and 66 review-required supports, with 142 abstentions and 0 high-risk supported items. | Parsing helps the recoverable subset with far less context, but it does not solve semantic-fidelity failures. |

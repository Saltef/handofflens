...
Because the lowest-performing cases were also dense in extracted claims, HandoffLens added a decomposition stress diagnostic rather than claiming a broad causal length effect. The diagnostic selected 50 failed exact-provenance evidence items from each of three held-out Cohere cells and compared five support policies: exact full-note matching, normalized full-note matching, line-span retrieval, section-filtered retrieval, and query-aware multi-span retrieval with a bounded greedy token-union fallback plus assertion-cue guarding. Across 150 deliberately difficult items, exact matching supported 0, normalized full-note matching supported 24, line-span retrieval supported 32, section-filtered retrieval supported 26, and assertion-guarded query-aware multi-span retrieval supported 83. A coherence audit then separated those 83 supported items into 54 auto-accepted supports and 29 review-required supports, with 0 high-risk supported items and 67 abstentions. Full-note normalization consumed about 1,504 source words when it succeeded; targeted line and query-aware retrieval consumed about 10.1 and 20.0 words. The design implication is not "long notes cause failure." It is narrower: many failures are recoverable by decomposing dense notes into auditable spans, but low-overlap items, assertion conflicts, medium-risk stitching cases, and semantic-fidelity questions still need abstention, entailment scoring, or review.

This is aligned with recent long-context findings rather than based only on project intuition. "Lost in the Middle" shows that even long-context models can underuse evidence depending on where it appears inside the prompt [10], and later positional-bias work argues that relevance can be distorted by context position rather than semantic importance alone [11]. For HandoffLens, that means the safest near-term design test is not a larger prompt window. It is query-aware evidence selection plus conservative abstention when selected spans are too diffuse, assertion-conflicting, or only weakly overlapping.
...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Assertion-guarded query-aware multi-span retrieval supported 83/150 worst selected items; coherence audit split these into 54 auto-accepted and 29 review-required supports, with 67 abstentions and 0 high-risk supported items. | Parsing helps the recoverable subset with far less context, but it does not solve semantic-fidelity failures. |
...
[11] Hsieh, C.-Y., Chuang, Y.-S., Li, C.-L., Wang, Z., Le, L., Kumar, A., Glass, J., Ratner, A., Lee, C.-Y., Krishna, R., & Pfister, T. (2024). Found in the Middle: Calibrating Positional Attention Bias Improves Long Context Utilization. Google Research. https://research.google/pubs/found-in-the-middle-calibrating-positional-attention-bias-improves-long-context-utilization/

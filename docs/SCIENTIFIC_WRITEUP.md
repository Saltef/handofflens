...
Because the lowest-performing cases were also dense in extracted claims, HandoffLens added a decomposition stress diagnostic rather than claiming a broad causal length effect. The diagnostic selected 200 failed exact-provenance evidence items from each of three held-out Cohere cells and compared five support policies: exact full-note matching, normalized full-note matching, line-span retrieval, section-filtered retrieval, and clause-aware query retrieval with a bounded greedy token-union fallback plus assertion-cue guarding. Across 600 deliberately difficult items, exact matching supported 0, normalized full-note matching supported 39, line-span retrieval supported 113, section-filtered retrieval supported 86, and assertion-guarded clause-aware query retrieval supported 395. A coherence audit then separated those 395 supported items into 238 auto-accepted supports and 157 review-required supports, with 0 high-risk supported items and 205 abstentions. Full-note normalization consumed about 1,662 source words when it succeeded; targeted line and query-aware retrieval consumed about 9.1 and 21.5 words. The latest revision made assertion-cue guarding target-aware: broad cues such as "no", "not", "without", and uncertainty terms block support only when their local scope overlaps the extracted item target, with a narrow lab/procedure exception for negative test results or uncomplicated procedures. A later strict low-overlap rescue recovered 3 more items in the expanded run, all review-required.

The same policy was then expanded to every available failed exact-provenance item selected by the runner across the three held-out cells. This full-pool validation contains 13,038 items and includes easier exact-match failures than the 600-item hard slice. Query-aware retrieval supported 11,001 items: 7,829 auto-accepted, 3,172 review-required, and 2,037 abstained. A stricter label-only risk guard converted 332 diffuse label-only unions into abstentions, leaving 0 high-risk supported items. The design implication is not "long notes cause failure." It is narrower: many failures are recoverable by decomposing dense notes into auditable spans, but low-overlap items, remaining assertion conflicts, medium-risk stitching cases, and semantic-fidelity questions still need abstention, entailment scoring, or review.

The next test reran the model itself rather than only rescoring existing outputs. A 20-case hard slice was selected from private records with at least five failed exact-provenance stress items per case; the slice had 545 to 6,476 source words and a median of about 10 failed stress items per 1,000 words. Four full-note Command A+ request modes were compared with candidate-first v4. JSON-schema full-note extraction completed 16/20 cases but only 2/16 completed cases passed the deterministic exact/span provenance gate. Plain JSON completed 0/20, strict tools completed 4/20, and flat tools completed 6/20. Candidate-first v4 completed and passed deterministic gates on 20/20 cases, with 716 exact source-backed evidence items. Token logprobs were captured for JSON/JSON-schema and candidate-first JSON-schema calls; Cohere did not support logprobs on tool-call requests, so tool-mode telemetry records that provider limitation. The important caveat is that candidate-first exact support is expected by construction: it emits materialized source candidates, so this is evidence for auditability and failure-mode control, not semantic completeness or clinical correctness.

A cross-provider schema ablation then tested a stricter provenance representation: generated free-text `source_quote` strings versus `evidence_span_ids` selected from a deterministic clause index. The 20 hard cases were run across two frozen providers, Cohere Command A+ and Anthropic Claude Haiku 4.5 through OpenRouter, with three repeats per model/arm cell. Cohere benefited from span IDs: quote-v2 support was 769/1192 items, 64.5% [61.8, 67.2], while span-ID-v5 support was 1127/1276, 88.3% [86.4, 90.0]. Minimal selection also helped Cohere, reaching 1131/1192, 94.9% [93.5, 96.0], on quote-v2 and 1185/1276, 92.9% [91.3, 94.2], on span-ID-v5. Haiku showed a negative provider interaction: quote-v2 support was 1709/1825, 93.6% [92.4, 94.7], but span-ID-v5 support dropped to 1453/2168, 67.0% [65.0, 69.0], and the minimal selector recovered only to 1934/2168, 89.2% [87.8, 90.4]. The key failure mode was over-selection: hosted structured-output compatibility required stripping local `maxItems` constraints, so Haiku produced 703 too-many-span-ID violations. Resolvable span-ID validity was 100% for successful span-ID runs because provider-side enums constrained IDs, but full v5 contract validity was only 52.8% for Haiku and 86.4% for Cohere. The result supports span IDs as a pointer-integrity design, not as a semantic-fidelity solution.

The item-count confound is explicit. Raw case gates are conjunctive, so an arm that emits more items gets more opportunities to fail. A volume-normalized paired check therefore compares only the first `min(reference item count, comparison item count)` items within the same model, case, and repeat. Under this matched-count control, Haiku quote-v2 still outperforms span-ID-v5: 1634/1747 versus 1145/1747 matched-item support, and 20/60 versus 3/60 matched case gates. Cohere shows the reverse: span-ID-v5 outperforms quote-v2 with 872/996 versus 658/996 matched-item support, and 24/59 versus 6/59 matched case gates. For minimal-selector variants, quote-v2 minimal remains slightly better than span-ID-v5 minimal at matched volume: Haiku 1650/1747 versus 1564/1747 item support, and Cohere 954/996 versus 929/996. This means the raw case-gate difference was confounded by extraction volume, but the provider-specific direction of the item-support result was not.

A deterministic cap-repair pass then tested whether the schema failures were recoverable without another model call. The repair drops unknown span IDs, reduces supported span sets to three or fewer using the minimal selector, removes invalid optional `surface_form` offsets, and routes unrepaired supported items to `insufficient_evidence` or `not_found`. On Cohere span-ID-v5, repaired support increased from 1127/1276 to 1224/1276, repaired case gate from 18/59 to 34/59, and repaired local v5 contract validity from 1102/1276 to 1276/1276. At matched volume versus quote-v2, repaired Cohere span-ID-v5 reached 957/996 item support and 39/59 matched case gates. Haiku also recovered substantially: raw span-ID-v5 support increased from 1453/2168 to 1978/2168 after repair, with local v5 contract validity rising from 1144/2168 to 2168/2168. However, under matched volume, repaired Haiku span-ID-v5 still trailed quote-v2 on item support, 1597/1747 versus 1634/1747, while roughly matching the case gate, 19/60 versus 20/60. The repair therefore improves auditability and contract compliance, especially for Cohere, but it is not evidence of semantic entailment or target recall.

The latest gate update addresses a reviewer-relevant confound in that result: case-level gates compound item-level failures. The provenance taxonomy now reports mean and median evidence items per completed case, the effective item count implied by item-level versus case-level pass rates, and case-gate outcomes stratified by extracted item count. Candidate-first materialization also records deterministic source candidate IDs, source offsets, and conservative near-duplicate evidence removal. A separate auditable-or-review-routed case metric counts only span-supported items plus weak-overlap items that can be cleanly withheld for review; low-overlap possible-fabrication items remain unresolved failures. This makes over-extraction visible instead of treating every case failure as equivalent.

The decomposition stress runner now adds a matched span-budget curve. For each selected hard evidence item, it compares lexical top-k span matching with a transparent reranker-style top-k matcher that adds section relevance and assertion-conflict penalties. The purpose is to test whether added context helps or only increases review risk. This is not yet an embedding-reranker result; it is the deterministic control needed before adding a neural reranker.

This is aligned with recent long-context findings rather than based only on project intuition. "Lost in the Middle" shows that even long-context models can underuse evidence depending on where it appears inside the prompt [10], and later positional-bias work argues that relevance can be distorted by context position rather than semantic importance alone [11]. NoLiMa further shows that literal-match retrieval can overstate long-context competence when the task requires latent association rather than surface overlap [12], while controlled long-context experiments report degradation even when relevant information is retrievable [13]. Recent long-context RAG baseline work also cautions that added pipeline complexity should be tested against simple structure-preserving retrieval under matched token budgets [14]. For HandoffLens, that means the safest near-term design test is not a larger prompt window. It is query-aware evidence selection plus conservative abstention when selected spans are too diffuse, assertion-conflicting, only weakly overlapping, or supported mainly by diffuse labels rather than specific quoted evidence.
...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Assertion-guarded clause-aware query retrieval supported 395/600 worst selected items; full-pool validation supported 11,001/13,038 failed exact-provenance items after label-risk abstention; both audits left 0 high-risk supported items. | Parsing helps the recoverable subset with far less context, but the remaining failures are still dominated by weak overlap, assertion conflicts, and review-required stitching cases. |
| Model-side hard20 diagnostic | Do request-mode changes or candidate-first parsing improve dense hard cases when the model is rerun? | JSON-schema full-note extraction completed 16/20 but only 2/16 completed cases passed deterministic gates; candidate-first v4 completed and passed 20/20. | The best next design is candidate-first evidence selection, not another full-note request-mode tweak; exact support remains by construction and requires factual review. |
| Case-gate arithmetic diagnostic | Are case failures driven partly by the number of extracted items? | Provenance reports now include item-count buckets, effective item-count estimates, deterministic source candidate IDs, conservative deduplication, and auditable-or-review-routed case metrics. | This exposes over-extraction and clean review-routing separately; it is not a replacement for factual labels. |
| Span-budget diagnostic | How does support change as more source spans are exposed? | Decomposition reports now include lexical top-k and transparent reranker-style top-k curves over 1, 2, 4, and 8 spans. | This makes context budget a measured tradeoff, but it is still a deterministic proxy rather than semantic entailment or a neural-reranker result. |
| Span-ID v5 cross-provider ablation | Are stable source-span IDs better than generated free-text quotes? | Cohere span-ID-v5 improved item support over quote-v2, while Haiku span-ID-v5 underperformed quote-v2; cap repair improved local contract validity and repaired support. | Raw case gates are volume-confounded. Cap repair improves auditability, not semantic entailment or target recall. |
...
- The decomposition stress diagnostics are selected from failed exact-provenance items. The 600-item hard slice emphasizes the lowest-performing records, while the 13,038-item full-pool run is a scaling check over all selected failed exact matches. Neither is a population estimate or proof that note length causally drives errors.
- The model-side hard20 diagnostic reruns the model on selected difficult private cases, but it has no gold extraction labels. Completion, exact quote support, span support, and deterministic gate pass rates are auditability measures, not precision/recall or clinical correctness.
- The auditable-or-review-routed metric is a triage metric. It separates weak-overlap review routing from strict failure, but it does not prove that the routed item is factually correct.
- The transparent reranker-style span-budget curve is a deterministic comparator, not a learned embedding/reranker benchmark.
- The span-ID v5 ablation measures pointer resolvability and local contract compliance, not semantic entailment. Resolvable span IDs are expected when provider-side enums are honored.
- Hosted-provider structured-output compatibility required stripping some local schema constraints, including the max-3 evidence-span cap. Deterministic cap repair now recovers many of those local contract failures, but live provider retry and semantic validation remain untested.
- Cross-arm raw case gates in the span-ID v5 ablation are not directly comparable because extraction volume differs by arm. Matched-count gates reduce this confound but still do not prove identical target selection or recall.
...
[12] Modarressi, A., Deilamsalehy, H., Dernoncourt, F., Bui, T., Rossi, R. A., Yoon, S., & Schuetze, H. (2025). NoLiMa: Long-Context Evaluation Beyond Literal Matching. *Proceedings of Machine Learning Research*, 267. https://proceedings.mlr.press/v267/modarressi25a.html

[13] Du, Y., Tian, M., Ronanki, S., Rongali, S. B., Bodapati, S. B., Galstyan, A., Wells, A., Schwartz, R., Huerta, E. A., & Peng, H. (2025). Context Length Alone Hurts LLM Performance Despite Perfect Retrieval. *Findings of EMNLP 2025*. https://aclanthology.org/2025.findings-emnlp.1264/

[14] Laitenberger, A., Manning, C. D., & Liu, N. F. (2025). Stronger Baselines for Retrieval-Augmented Generation with Long-Context Language Models. *EMNLP 2025*. https://aclanthology.org/2025.emnlp-main.1656/

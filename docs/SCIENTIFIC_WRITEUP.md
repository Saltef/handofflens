...
| Decomposition stress diagnostic | Do parsing/chunking policies help on the lowest exact-provenance items? | Assertion-guarded clause-aware query retrieval supported 395/600 worst selected items; coherence audit split these into 238 auto-accepted and 157 review-required supports, with 205 abstentions and 0 high-risk supported items. | Parsing helps the recoverable subset with far less context, but the remaining failures are still dominated by weak overlap, low overlap, and review-required stitching cases. |

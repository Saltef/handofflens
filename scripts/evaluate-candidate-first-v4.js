...
const { dedupeEvidenceInPlace } = require("./evidence-dedup");
...
async function runCase(testCase, source, index) {
  ...
  let selected = [...deterministicSelections, ...consensusSelections(selectionRuns, selectionRepeats)], audit = [], deduplication = null;
  const validated = validateSelections(selected, index.candidates); selected = validated.selected; audit = validated.audit;
  let materialized = materialize(testCase, selected, index.candidates);
  let extraction = materialized.extraction; deduplication = materialized.deduplication;
  ...
  if (missingDomains.length) {
    ...
    try {
      ...
      selected = dedupeSelections([...selected, ...checked.selected]); audit.push(...checked.audit); materialized = materialize(testCase, selected, index.candidates); extraction = materialized.extraction; deduplication = materialized.deduplication; missingDomains = detectedMissingDomains(index, extraction);
...
function materialize(testCase, selections, candidates) { const byId = Object.fromEntries(candidates.map((x) => [x.candidate_id, x])), extraction = emptyExtraction(testCase), deduplication = { removed_items: 0, by_path: {}, groups: [] }; for (const selection of selections) { const candidate = byId[selection.candidate_id]; if (!candidate) continue; const item = { label: extractiveLabel(candidate), rationale: candidate.origin === "section_entry" ? `Parsed from explicit ${candidate.domain_hint.replace(/_/g, " ")} section.` : "Selected from an explicit source candidate.", source_quote: candidate.source_quote, source_candidate_id: candidate.candidate_id, source_span_ids: [candidate.candidate_id], source_span: { index: "canonical-source-map-v1", origin: candidate.origin, domain_hint: candidate.domain_hint, canonical_start: candidate.canonical_start, canonical_end: candidate.canonical_end, original_start: candidate.original_start, original_end: candidate.original_end } }; const target = targetList(extraction, selection.category); target.push(item); } for (const [pathValue, list] of allLists(extraction)) { const audit = dedupeEvidenceInPlace(list); if (audit.removed_items) { deduplication.removed_items += audit.removed_items; deduplication.by_path[pathValue] = audit.removed_items; deduplication.groups.push(...audit.groups.map((group) => ({ path: pathValue, ...group }))); } } return { extraction, deduplication }; }
...
function summarize(records) { const live = records.filter((x) => !x.dry_run), evidenceCounts = live.map((x) => countEvidence(x.extraction || emptyExtraction({}))).filter(Number.isFinite); return { attempted: records.length, final_gate_passed: live.filter((x) => x.final_gate?.valid).length, abstained: live.filter((x) => x.abstention?.required).length, recovered: live.filter((x) => x.recovery?.triggered).length, candidate_overflow_cases: records.filter((x) => (x.candidate_index?.overflow?.count || x.overflow?.count || 0) > 0).length, evidence_items: evidenceCounts.reduce((sum, value) => sum + value, 0), mean_evidence_items_per_case: mean(evidenceCounts), median_evidence_items_per_case: median(evidenceCounts), evidence_item_count_buckets: countBy(evidenceCounts.map(itemCountBucket)), deduplicated_evidence_items: live.reduce((n, x) => n + (x.deduplication?.removed_items || 0), 0), median_section_coverage: median(live.map((x) => x.section_coverage?.rate).filter(Number.isFinite)), median_selection_jaccard: median(live.map((x) => x.selection_agreement?.mean_jaccard).filter(Number.isFinite)), minimum_selection_jaccard: minimum(live.map((x) => x.selection_agreement?.mean_jaccard).filter(Number.isFinite)), total_calls: live.reduce((n, x) => n + x.calls.length, 0), stage_failures: countBy(live.flatMap((x) => x.stage_errors).map((x) => x.stage)) }; }
...
function itemCountBucket(count) { if (count < 5) return "<5"; if (count < 10) return "5-9"; if (count < 15) return "10-14"; if (count < 25) return "15-24"; return "25+"; }

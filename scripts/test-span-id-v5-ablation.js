...
const capRepairItem = {
  ...
  surface_form: { span_id: "S2", token_start: 0, token_end: 99 },
  entailment_scored: false,
  entailment_score: null,
};
const capRepair = repairSpanIdEvidenceItem(capRepairItem, sourceText, spanIndex);
assert.equal(capRepair.needed, true);
assert.equal(capRepair.supported, true);
assert.equal(capRepair.validation.valid, true);
assert.equal(capRepair.item.evidence_span_ids.length <= 3, true);
assert.equal("surface_form" in capRepair.item, false);
assert.equal(capRepair.actions.includes("repair_too_many_span_ids"), true);
assert.equal(capRepair.actions.includes("drop_invalid_surface_form"), true);

const capRepairRecord = scoreAblationRecord({
  execution_index: 4,
  model: "cohere-aplus:command-a-plus-05-2026",
  arm: "span_id_v5",
  repeat: 1,
...
assert.equal(capRepairRecord.scoring.item_support_rate.numerator, 0);
assert.equal(capRepairRecord.scoring.cap_repaired_item_support_rate.numerator, 1);
assert.equal(capRepairRecord.scoring.cap_repaired_case_gate_rate.numerator, 1);
assert.equal(capRepairRecord.scoring.cap_repaired_v5_contract_validity_rate.numerator, 1);
assert.equal(capRepairRecord.scoring.cap_repair_action_counts.repair_too_many_span_ids, 1);

const summary = summarizeAblation([quoteRecord, minimalQuoteRecord, spanRecord, invalidSpanRecord, capRepairRecord]);
...
assert.equal(summary["cohere-aplus:command-a-plus-05-2026||span_id_v5"].cap_repaired_item_support_rate.numerator, 1);
assert.equal(summary["cohere-aplus:command-a-plus-05-2026||span_id_v5"].cap_repaired_v5_contract_validity_rate.numerator, 1);
...
assert.equal(volumeComparisons[comparisonKey].comparison_cap_repaired_case_gate_at_matched_count.numerator, 1);
assert.equal(volumeComparisons[comparisonKey].comparison_cap_repaired_item_support_at_matched_count.numerator, 1);
...
assert.equal(regeneratedSummary.schema_ablation.cap_repair.policy.includes("minimal selector"), true);
assert.equal(regeneratedSummary.schema_ablation.aggregate_results["cohere-aplus:command-a-plus-05-2026||span_id_v5"].cap_repaired_item_support_rate.numerator, 1);
assert.equal(regeneratedSummary.schema_ablation.volume_normalized_comparisons[comparisonKey].comparison_cap_repaired_case_gate_at_matched_count.numerator, 1);
...
console.log("PASS span-ID v5 ablation (47 assertions)");

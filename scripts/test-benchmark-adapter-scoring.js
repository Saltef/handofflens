...
assert.equal(repairReport.selected_method, "compact_extractive");
assert.equal(repairReport.method_selection.policy, "prespecified");
assert.equal(repairReport.method_selection.ranking_is_descriptive, true);
assert.match(repairReport.method_selection.leakage_caveat, /same-row selection/);
assert.match(repairReport.selection_rule, /pre-specified/);
...
console.log("PASS benchmark adapter and scoring checks (50 assertions)");

...
function materializeExtractionWithAudit(pointerExtraction, index, options = {}) {
  ...
  if (Array.isArray(output.handoff_atoms)) output.handoff_atoms = convertList(output.handoff_atoms, "handoff_atoms");
...
function materializeItem(item, index, path, options, audit) {
  ...
  const materialized = { label: String(item.label || ""), rationale: String(item.rationale || ""), source_quote: segments.map((segment) => segment.text).join("\n") };
  if (typeof item.safety_type === "string") materialized.safety_type = item.safety_type;
  for (const key of ["atom_id", "atom_type", "instruction_kind", "action", "target", "time_window", "threshold", "owner"]) {
    if (typeof item[key] === "string") materialized[key] = item[key];
  }
  if (Array.isArray(item.derived_views)) materialized.derived_views = item.derived_views.filter((view) => typeof view === "string");
  return materialized;
}
